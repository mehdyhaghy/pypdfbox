...
| `tests/util/upstream/test_date_util.py` | 3.0.x | `pdfbox/src/test/java/org/apache/pdfbox/util/TestDateUtil.java` (wave 1360 agent D; 86 tests + 7 skips covering PDFBOX-598 round-trip, full `checkParse` battery for digit-only PDF + ISO shapes, `BAD`-sentinel rejection rows, `testToString` over Pgh/Berlin/Aruba/Jamaica/Adelaide offsets, full `testParseTZ` 23-row table, full `testFormatTZoffset` 18-row table; skips: locale-sensitive `SimpleDateFormat` month-name / weekday / slash-separated shapes — Java JVM locale dictionaries are not ported, and ambiguous big-endian forms remain SimpleDateFormat heuristic territory) |
