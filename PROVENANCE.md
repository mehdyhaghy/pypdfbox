...
| `tests/xmpbox/schema/upstream/test_exif_xmp.py` | 3.0.x | `xmpbox/src/test/java/org/apache/xmpbox/schema/TestExifXmp.java` (wave 1360 agent A; ``testNonStrict`` against bundled ``validxmp/exif.xmp`` fixture; ``testGenerate`` adapted to exercise the serializer with a simple-property ExifSchema since OECFType is not yet ported, plus a skipped placeholder for the structured OECF oracle) |
