...
class FDFDocument:
    ...
    def set_xfdf(self, xfdf: object) -> None:
        ...
        Mirrors the upstream method shape so callers get a clear pointer
        rather than ``AttributeError``.
        """
        raise NotImplementedError(
            "XFDF (XML Forms Data Format) is not yet supported — "
            "see CHANGES.md / open a feature request."
        )

    def save_xfdf(
        self,
        target: str | os.PathLike[str] | BinaryIO,  # noqa: ARG002 — parity stub
    ) -> None:
        """XFDF serialization is deferred to a later wave."""
        raise NotImplementedError(
            "XFDF (XML Forms Data Format) is not yet supported — "
            "see CHANGES.md / open a feature request."
        )
