...
class PDUnderlineAppearanceHandler(PDAbstractAppearanceHandler):
    ...
    def generate_normal_appearance(self) -> None:
        ...
        (PDUnderlineAppearanceHandler.java:47)."""
        from ..pd_annotation_underline import PDAnnotationUnderline

        annotation = self.get_annotation()
        if not isinstance(annotation, PDAnnotationUnderline):
            return
        ...
        with self.get_normal_appearance_as_content_stream() as cs:
            ...
            # Upstream passes the typed PDColor to setStrokingColor
            # (PDUnderlineAppearanceHandler.java:97), so the appearance stream
            # emits "/DeviceRGB CS r g b SC" (colour-space select + components
            # + SC), not the device-shorthand RG. Mirror that exactly by
            # wrapping the raw /C components first.
            cs.set_stroking_color(
                self._pd_color_from_components(stroke_components)
            )
            ...
            for i in range(len(paths_array) // 8):
                base = i * 8
                # Adobe doesn't use the lower coordinate for the line, it
                # uses lower + delta / 7 (java line 110).
                len0 = math.hypot(
                ...
                if len0 != 0:
                    x0 += (paths_array[base] - paths_array[base + 4]) / len0 * len0 / 7
                    y0 += (paths_array[base + 1] - paths_array[base + 5]) / len0 * (
                        len0 / 7
                    )
                len1 = math.hypot(
                ...
                if len1 != 0:
                    x1 += (paths_array[base + 2] - paths_array[base + 6]) / len1 * len1 / 7
                    y1 += (paths_array[base + 3] - paths_array[base + 7]) / len1 * len1 / 7
                cs.move_to(x0, y0)
                cs.line_to(x1, y1)
            cs.stroke()

    def generate_rollover_appearance(self) -> None:
        # No rollover appearance (PDUnderlineAppearanceHandler.java:145)
        return None

    def generate_down_appearance(self) -> None:
        # No down appearance (PDUnderlineAppearanceHandler.java:151)
        return None


__all__ = ["PDUnderlineAppearanceHandler"]
