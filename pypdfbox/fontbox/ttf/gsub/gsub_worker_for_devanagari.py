"""``GsubWorkerForDevanagari`` — Devanagari script GSUB worker.

Mirrors ``org.apache.fontbox.ttf.gsub.GsubWorkerForDevanagari`` from
upstream Apache PDFBox 3.0.x. Adjusts reph position, repositions
before-half (i-matra) glyphs, optionally synthesizes ``rkrf`` from
``vatu`` when the font lacks an explicit ``rkrf`` feature, then applies
...
# Feature order from GsubWorkerForDevanagari.java:50.
...
# Reph glyphs: RA (U+0930) + VIRAMA (U+094D). From GsubWorkerForDevanagari.java:55.
_REPH_CHARS: tuple[str, ...] = ("र", "्")
# Glyphs that should precede reph (AA-matra, II-matra). From line 57.
_BEFORE_REPH_CHARS: tuple[str, ...] = ("ा", "ी")
# Devanagari vowel sign I. From line 60.
_BEFORE_HALF_CHAR = "ि"


class GsubWorkerForDevanagari(GsubWorker):
    """Devanagari-script worker with reph adjustment and i-matra reposition."""
    ...
    def apply_rkrf_feature(
        ...
        """Mirrors GsubWorkerForDevanagari.java:112."""
        ...
        # Find the first cluster of length > 1 and grab its second glyph
        # as the rkrf replacement candidate.
        rkrf_replacement = 0
    ...
    def adjust_reph_position(self, original_glyph_ids: list[int]) -> list[int]:
        ...
        Mirrors GsubWorkerForDevanagari.java:158.
        """
...
__all__ = ["GsubWorkerForDevanagari"]
