"""``k`` — Set the non-stroking color space to DeviceCMYK and set the color.

Mirrors ``org.apache.pdfbox.contentstream.operator.color.SetNonStrokingDeviceCMYKColor``
...
class SetNonStrokingDeviceCMYKColor(SetNonStrokingColor):
    """``k`` — switch the non-stroking color space to DeviceCMYK, then sc."""

    OPERATOR_NAME = OperatorName.NON_STROKING_CMYK
...
__all__ = ["SetNonStrokingDeviceCMYKColor"]
