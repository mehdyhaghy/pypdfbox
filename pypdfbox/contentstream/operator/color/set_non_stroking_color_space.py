...
class SetNonStrokingColorSpace(OperatorProcessor):
    ...
    def process(self, operator: Operator, operands: list[COSBase]) -> None:
        ...
        setter = getattr(graphics_state, "set_non_stroking_color_space", None)
        if setter is not None:
            setter(color_space)
        else:
            self._set_attr(
                graphics_state, "non_stroking_color_space", color_space
            )
        # Mirror upstream: also reset the non-stroking colour to the new
        # colour space's initial colour value.
        get_initial = getattr(color_space, "get_initial_color", None)
        if get_initial is not None:
            initial = get_initial()
            color_setter = getattr(
                graphics_state, "set_non_stroking_color", None
            )
            if color_setter is not None:
                color_setter(initial)
            else:
                self._set_attr(graphics_state, "non_stroking_color", initial)
            engine_setter = getattr(context, "set_non_stroking_color", None)
