...
/**
 * Live oracle probe for CCITT Group 3/4 fax image decode.
 *
 * Usage: java -cp <pdfbox-app.jar>:<build> CcittImgProbe input.pdf
 *
 * Walks every page's PDResources.getXObjectNames(), and for each
 * PDImageXObject whose /Filter is CCITTFaxDecode (the only images this
 * probe's fixtures contain) emits one UTF-8 line to stdout:
 *
 *   "ccitt page <p> name <name> w <w> h <h> bpc <bpc> cs <colorspace> "
 ...
 * raster (PDImageXObject.getImage()), row-major, matching ImageExtractProbe
 * / RenderProbe cell mapping. For a bilevel CCITT raster the cell averages
 * are near-identical across Java2D and Pillow, so a small tolerance survives
 * sub-pixel block-boundary rounding while still catching gross divergences
 * (wrong dims, inverted BlackIs1 polarity, garbled raster).
 */
public final class CcittImgProbe {
    ...
    private static void emit(PrintStream out, int page, String name,
        ...
        sb.append("ccitt page ").append(page)
