...
/**
 * Live oracle probe for the stream-filter decode surface.
 *
 * Reads raw (encoded) bytes from a file, decodes them with Apache PDFBox's
 * {@link Filter} for the named filter, and writes the DECODED bytes to stdout
 * verbatim (raw bytes — the harness's {@code run_probe} returns them as-is).
 *
 * Usage:
 *   java -cp ... FilterDecodeProbe encoded.bin FlateDecode
 *   java -cp ... FilterDecodeProbe encoded.bin FlateDecode Predictor=12,Columns=4
 *   java -cp ... FilterDecodeProbe encoded.bin LZWDecode  EarlyChange=0
 *
 ...
 *             AHx, RL) also resolve via FilterFactory.
 ...
 *
 * The COSDictionary handed to {@code Filter.decode} mirrors a stream dict:
 * a single-name /Filter plus a single /DecodeParms dictionary. The filter's
 * own {@code getDecodeParams(dict, 0)} then resolves the predictor params,
 * exactly as it would for a real PDF stream object.
 */
public final class FilterDecodeProbe {
