...
def test_wave573_xref_shape_check_preserves_cursor_and_recognizes_xref_stream() -> None:
    ...
    parser = _parser(pdf)
    parser._cos_parser = COSParser(parser._src)
    parser._src.seek(len(pdf) - 5)

    assert parser._xref_section_starts_at(xref_offset)
    assert parser._src.get_position() == len(pdf) - 5
    assert not parser._xref_section_starts_at(-1)
    assert not parser._xref_section_starts_at(len(pdf))
...
def test_wave573_lenient_recovery_finds_nearby_xref_stream_offset() -> None:
    ...
    assert parser._recover_xref_offset_if_needed(xref_offset + 2) == xref_offset

    parser.set_lenient(False)
    assert parser._recover_xref_offset_if_needed(xref_offset + 2) == xref_offset + 2
...
def test_wave573_resolve_dict_entry_returns_loaded_direct_and_skips_compressed() -> None:
    ...
    try:
        ...
        assert parser._resolve_dict_entry(
