...
def test_constructor_with_resources_dic(tk_root) -> None:
    ...
    assert pane._resources is not None
...
def test_build_header_back_compat_alias(tk_root) -> None:
    ...
    pane._build_header(pane._stream.get_filter_list(), Stream.DECODED)
...
def test_on_filter_changed_image_branch_triggers_image_dispatch(
    ...
    pane._filter_combo.set(Stream.IMAGE)
    pane._on_filter_changed()
    assert called == [True]
    # Only one tab remains after rebuild.
    assert len(pane._notebook.tabs()) == 1
...
def test_on_filter_changed_decoded_branch_rebuilds_three_tabs(
    ...
    assert len(pane._notebook.tabs()) == 3
...
def test_on_filter_changed_text_hex_branch_for_non_content_stream(
    ...
    pane._filter_combo.set("some-other-filter")
    pane._on_filter_changed()
    assert seen == ["some-other-filter"]
    assert len(pane._notebook.tabs()) == 2
...
def test_rebuild_notebook_resets_tabs(tk_root) -> None:
    ...
    pane.init()
    initial_count = len(pane._notebook.tabs())
    assert initial_count == 3
    # Rebuild with two tabs.
    pane._rebuild_notebook([
        (pane._raw_view.get_stream_panel(), "A"),
        (pane._hex_view.get_pane(), "B"),
    ])
    assert len(pane._notebook.tabs()) == 2
...
def test_content_stream_segments_oserror_returns_none(
    ...
    result = pane._content_stream_segments(b"foo")
...
def test_emitter_handles_cos_boolean() -> None:
    ...
    emitter._write_operand(COSBoolean.TRUE)
...
def test_emitter_handles_cos_float_and_cos_integer() -> None:
    emitter = _ContentStreamEmitter()
    emitter._write_operand(COSInteger.get(42))
    emitter._write_operand(COSFloat(3.14))
    text = "".join(s for s, _ in emitter.segments)
    assert "42" in text
    assert "3.14" in text
...
def test_emitter_handles_cos_dictionary_and_cos_null() -> None:
    ...
    emitter._write_operand(d)
    emitter._write_operand(COSNull.NULL)
    text = "".join(s for s, _ in emitter.segments)
    assert "<<" in text and ">>" in text
    assert "null" in text
...
def test_emitter_indent_pushed_on_q_popped_on_qq() -> None:
    ...
    emitter._add_operator(q)
    assert emitter._indent == 1
    emitter._add_operator(qq)
    assert emitter._indent == 0
...
def test_emitter_indent_push_on_bmc_pop_on_emc() -> None:
    ...
    emitter._add_operator(bmc)
    assert emitter._indent == 1
    emitter._add_operator(emc)
    assert emitter._indent == 0
...
def test_emitter_write_indent_emits_spaces_at_non_zero_indent() -> None:
    emitter = _ContentStreamEmitter()
    emitter._indent = 2
    emitter._need_indent = True
    emitter._write_indent()
    text = "".join(s for s, _ in emitter.segments)
