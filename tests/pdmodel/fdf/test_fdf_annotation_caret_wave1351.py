"""Wave 1351 coverage boost: ``FDFAnnotationCaret`` /RD lenient coercion.
...
def test_get_fringe_coerces_non_numeric_entries_to_zero_rectangle() -> None:
    ...
    arr = COSArray()
    for _ in range(4):
        arr.add(COSName.A)  # non-numeric — coerces to 0.0
    annot.set_item(COSName.get_pdf_name("RD"), arr)
    caret = FDFAnnotationCaret(annot)
    assert caret.get_fringe() == PDRectangle(0.0, 0.0, 0.0, 0.0)
