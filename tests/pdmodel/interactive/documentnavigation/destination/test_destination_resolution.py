...
def test_find_page_number_returns_minus_one_for_unresolved_page_dict() -> None:
    """A page COSDictionary still needs document/page-tree context."""
...
def test_find_page_number_resolves_page_dict_against_document() -> None:
    ...
    assert dest.find_page_number(doc.get_pages()) == 1
    # Without a document context we still return -1 (page-dict path).
    assert dest.find_page_number() == -1


def test_find_page_number_resolves_indirect_page_object_against_document() -> None:
    doc = PDDocument()
    pages = [PDPage(), PDPage(), PDPage()]
    for page in pages:
        doc.add_page(page)

    page_ref = COSObject(9, 0, resolved=pages[2].get_cos_object())
    arr = COSArray([page_ref, COSName.get_pdf_name("XYZ")])
    dest = PDPageXYZDestination(arr)

    assert dest.get_page() is pages[2].get_cos_object()
    assert dest.find_page_number(doc) == 2
    assert dest.retrieve_page_number(doc.get_pages()) == 2


def test_find_page_number_returns_minus_one_for_missing_page() -> None:
    doc = PDDocument()
    doc.add_page(PDPage())

    orphan_page = PDPage()
    arr = COSArray([orphan_page.get_cos_object(), COSName.get_pdf_name("Fit")])
    dest = PDPageFitDestination(arr)
