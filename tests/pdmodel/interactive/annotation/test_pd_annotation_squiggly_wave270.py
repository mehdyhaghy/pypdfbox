"""Wave 270 — pdmodel/interactive/annotation/PDAnnotationSquiggly parity gaps.
...
def test_default_constructor_sets_subtype_wave270() -> None:
    ...
    assert annotation.get_subtype() == "Squiggly"
...
def test_extends_text_markup_wave270() -> None:
    """Inheritance chain stays Markup → TextMarkup → Squiggly."""
    annotation = PDAnnotationSquiggly()
...
def test_constructor_with_dict_preserves_subtype_wave270() -> None:
    d = COSDictionary()
    d.set_name(COSName.SUBTYPE, "Squiggly")  # type: ignore[attr-defined]
    annotation = PDAnnotationSquiggly(d)
    assert annotation.get_subtype() == "Squiggly"
...
def test_construct_appearances_without_handler_is_noop_wave270() -> None:
    """The built-in ``PDSquigglyAppearanceHandler`` isn't ported yet, so
