...
def test_multiline_break() -> None:
    ...
    Asserts that the line count and per-line character count of the
    regenerated appearance match the original Acrobat-saved wrap
    exactly. Wave 1375 restored the strict parity assertion after the
    wrap engine was upgraded to use real per-glyph widths plus the
    PDFBOX-5049 / PDFBOX-6082 force-split fallback for words wider than
    the rect.
    """
    with PDDocument.load(_FIXTURE_DIR / "PDFBOX-3835-input-acrobat-wrap.pdf") as doc:
        ...
        acrobat_lines = _text_lines_from_appearance(field_input)
        ...
        assert len(pdfbox_lines) == len(acrobat_lines), (
            "Number of lines generated by pypdfbox shall match Acrobat: "
            f"acrobat={acrobat_lines!r} pypdfbox={pdfbox_lines!r}"
        )
        for i, (acrobat_line, pdfbox_line) in enumerate(
            zip(acrobat_lines, pdfbox_lines, strict=False)
        ):
            assert len(pdfbox_line) == len(acrobat_line), (
                f"Line {i}: number of characters generated by pypdfbox "
                f"shall match Acrobat: acrobat={acrobat_line!r} "
                f"pypdfbox={pdfbox_line!r}"
            )
