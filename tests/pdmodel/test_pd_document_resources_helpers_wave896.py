...
def test_wave896_write_bytes_to_file_like_target() -> None:
    ...
    PDDocument._write_bytes_to_target(b"file-like", sink)

    assert sink.getvalue() == b"file-like"


def test_wave896_write_bytes_to_path_target(tmp_path) -> None:
    target = tmp_path / "signed-output.pdf"

    PDDocument._write_bytes_to_target(b"path-target", target)
...
def test_wave896_extract_bracketed_returns_disjoint_signed_ranges() -> None:
    ...
    assert PDDocument._extract_bracketed(data, [0, 4, 8, 4]) == b"abcdefgh"
