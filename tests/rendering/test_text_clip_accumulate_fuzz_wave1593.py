...
def _commit_renderer(
    ...
    *,
    ...
    r = _bare_renderer(gs)
    r._image = Image.new("RGBA", size, (0, 0, 0, 0))
    r._text_clip_paths = []
...
def test_multiple_clip_glyphs_accumulate_in_order() -> None:
    ...
    assert len(r._text_clip_paths) == 5
...
def test_accumulate_with_non_skia_path_is_dropped() -> None:
    ...
    r = _commit_renderer()
    r._accumulate_text_clip_path(object(), (1, 0, 0, 1, 0, 0))
...
def test_single_clip_glyph_commits_at_et() -> None:
    ...
    r._text_clip_paths = [_rect_path(10, 10, 30, 30)]
    r._commit_text_clip()
    m = r._gs.clip_mask
...
def test_commit_intersects_with_existing_gs_clip_at_et() -> None:
    ...
    r._text_clip_paths = [_rect_path(5, 5, 55, 25)]
    r._commit_text_clip()
    m = r._gs.clip_mask
...
def _paint_wired(mode: int, *, clip_mask: Any = None) -> tuple[Any, _PaintRec]:
    ...
    gs.clip_mask = clip_mask
    r = _bare_renderer(gs)
    r._image = Image.new("RGBA", (40, 40), (0, 0, 0, 0))
    r._text_clip_paths = []
    rec = _PaintRec()
    r._paint_glyph_path_direct = rec.direct_paint
    r._paint_glyph_path_through_clip = rec.through_paint
    return r, rec
...
def test_et_commits_exactly_once_and_resets_buffer() -> None:
    r = _commit_renderer((40, 40))
    r._text_clip_paths = [_rect_path(5, 5, 25, 25)]
    calls: list[bool] = []
    real_commit = r._commit_text_clip

    def _counting() -> None:
        calls.append(True)
        real_commit()

    r._commit_text_clip = _counting
    _stub_knockout(r)
    r._op_end_text(None, [])
...
def test_second_text_object_clip_narrows_first() -> None:
    ...
    r._op_begin_text(None, [])
    r._text_clip_paths = [_rect_path(0, 0, 40, 30)]
    r._op_end_text(None, [])
    # Object 2: clip to the right 20..60 → intersection is 20..40.
    r._op_begin_text(None, [])
    r._text_clip_paths = [_rect_path(20, 0, 60, 30)]
    r._op_end_text(None, [])
    m = r._gs.clip_mask
