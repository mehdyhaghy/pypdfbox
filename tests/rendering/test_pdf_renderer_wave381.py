...
def test_short_operands_leave_state_and_path_unchanged() -> None:
    doc, renderer = _prepared_renderer()
    try:
        renderer._gs.stroke_rgb = (7, 8, 9)
        renderer._gs.fill_rgb = (11, 12, 13)
        renderer._gs.line_width = 3.0
        ...
        assert renderer._gs.stroke_rgb == (7, 8, 9)
        assert renderer._gs.fill_rgb == (11, 12, 13)
        assert renderer._gs.line_width == 3.0
...
def test_pattern_shading_and_extgstate_resolution_error_branches(caplog: Any) -> None:
    ...
    doc, renderer = _prepared_renderer()
    renderer._resources = _BadResources()
    ...
    try:
        ...
        assert renderer._resolve_pattern_operand([]) is None
        assert renderer._resolve_pattern_operand([COSFloat(1.0)]) is None
        assert renderer._resolve_pattern_operand([COSName.get_pdf_name("P0")]) is None
