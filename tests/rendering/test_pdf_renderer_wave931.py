...
from tests.rendering import test_pdf_renderer_wave532 as wave532


def test_wave532_make_doc_removes_existing_pages(monkeypatch: Any) -> None:
    class FakeDocument:
    ...
    monkeypatch.setattr(wave532, "PDDocument", FakeDocument)

    doc, page = wave532._make_doc()  # noqa: SLF001
...
def test_wave532_image_xobject_stub_accessors_are_exercised(
    monkeypatch: Any,
) -> None:
    def decode_stub(_renderer: Any, image_xobject: Any) -> Image.Image:
        ...
        with image_xobject.create_input_stream(stop_filters=["JPXDecode"]) as stream:
            assert isinstance(stream, io.BytesIO)
            return Image.open(stream).convert("RGB")

    monkeypatch.setattr(wave532.PDFRenderer, "_decode_image_xobject", decode_stub)

    wave532.test_decode_image_xobject_jpx_path_uses_encoded_stream_payload()
