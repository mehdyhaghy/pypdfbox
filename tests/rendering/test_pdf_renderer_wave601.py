...
def test_rect_starts_new_subpath_and_close_resets_current_point() -> None:
    ...
    try:
        ...
        assert renderer._current_point == (1.0, 2.0)

        renderer.process_operator("re", _nums(3.0, 4.0, 2.0, 1.0))

        assert renderer._current_point == (3.0, 4.0)
        assert renderer._subpaths[-1] == [
            ("M", 3.0, 4.0),
            ("L", 5.0, 4.0),
            ("L", 5.0, 5.0),
            ("L", 3.0, 5.0),
...
def test_color_and_line_width_operators_ignore_short_inputs_and_clamp() -> None:
    ...
    try:
        ...
        assert renderer._gs.stroke_rgb == (255, 0, 0)
        assert renderer._gs.fill_rgb == (128, 128, 128)
        assert renderer._gs.line_width == 0.0

        renderer.process_operator("w", _nums(2.25))
        assert renderer._gs.line_width == 2.25
    finally:
        _finish(renderer)
        doc.close()
