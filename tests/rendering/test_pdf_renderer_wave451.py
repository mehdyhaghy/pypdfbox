...
def test_save_restore_graphics_state_operator_keeps_base_state() -> None:
    ...
    try:
        ...
        renderer._gs.line_width = 4.0

        renderer.process_operator("q", [])
        renderer._gs.fill_rgb = (9, 9, 9)
        renderer._gs.line_width = 10.0

        assert len(renderer._gs_stack) == 2
        renderer.process_operator("Q", [])
        renderer.process_operator("Q", [])

        assert len(renderer._gs_stack) == 1
        assert renderer._gs.fill_rgb == (1, 2, 3)
        assert renderer._gs.line_width == 4.0
    finally:
        _finish(renderer)
        doc.close()
...
def test_quoted_text_operator_sets_spacing_moves_line_and_advances() -> None:
    ...
    try:
        ...
        renderer._gs.text_leading = 12.0
        ...
        assert renderer._gs.text_wordspace == 5.0
        assert renderer._gs.text_charspace == 2.0
        assert renderer._gs.text_matrix[4:] == (41.0, -12.0)
    finally:
        _finish(renderer)
        doc.close()
...
def test_show_inline_image_uses_helper_image_before_legacy_decoder() -> None:
    ...
    try:
        ...
        assert renderer._image.getpixel((1, 1)) == (200, 10, 20)
