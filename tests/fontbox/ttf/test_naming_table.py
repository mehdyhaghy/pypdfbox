...
def test_pdfbox_camelcase_get_name_aliases() -> None:
    blob = _build_name_table([
        (
            NameRecord.PLATFORM_WINDOWS,
            NameRecord.ENCODING_WINDOWS_UNICODE_BMP,
    ...
    assert t.getName(NameRecord.NAME_FONT_FAMILY_NAME) == t.get_name(
        NameRecord.NAME_FONT_FAMILY_NAME
    )
    assert t.getName(
        NameRecord.NAME_COPYRIGHT,
    ...
    assert t.getNameRecords() is t.get_name_records()


def test_pdfbox_camelcase_metadata_aliases_delegate_to_snake_case() -> None:
    german = 0x0407
    blob = _build_name_table([
        (
            NameRecord.PLATFORM_WINDOWS,
        ...
        (
            ...
            "Alias Sans 1.0".encode("utf-16-be"),
        ),
        (
            NameRecord.PLATFORM_WINDOWS,
            NameRecord.ENCODING_WINDOWS_UNICODE_BMP,
            ...
            "Alias Sans Regular".encode("utf-16-be"),
        ),
        (
            NameRecord.PLATFORM_WINDOWS,
            NameRecord.ENCODING_WINDOWS_UNICODE_BMP,
        ...
        (
            ...
            " AliasSans-Regular ".encode("utf-16-be"),
        ),
        (
            NameRecord.PLATFORM_WINDOWS,
            NameRecord.ENCODING_WINDOWS_UNICODE_BMP,
        ...
        (
            ...
            german,
            NameRecord.NAME_FULL_FONT_NAME,
            "Alias Sans Deutsch".encode("utf-16-be"),
        ),
    ])
    t = _read(blob)

    assert t.getCopyright() == t.get_copyright() == "Copyright"
    assert t.getFontFamily() == t.get_font_family() == "Alias Sans"
    assert t.getFontSubFamily() == t.get_font_sub_family() == "Regular"
    assert t.getUniqueId() == t.get_unique_id() == "Alias Sans 1.0"
    assert t.getFullName() == t.get_full_name() == "Alias Sans Regular"
    assert t.getVersion() == t.get_version() == "Version 1.0"
    assert t.getPostScriptName() == t.get_post_script_name() == "AliasSans-Regular"
    assert t.getTrademark() == t.get_trademark() == "Alias(TM)"
    assert t.getFullName(german) == t.get_full_name(german) == "Alias Sans Deutsch"
