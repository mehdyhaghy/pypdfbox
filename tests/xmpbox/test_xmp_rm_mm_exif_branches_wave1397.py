...
def test_rights_build_owners_array_skips_non_string_items() -> None:
    ...
    schema._properties["Owner"] = ["Alice", 42, "Bob"]
    arr = schema._build_owners_array()
...
def test_rights_build_usage_terms_lang_alt_without_xdefault() -> None:
    ...
    schema._properties["UsageTerms"] = {
        "en-US": "License A",
        "fr-FR": "License B",
    }
    la = schema._build_usage_terms_lang_alt()
...
def test_media_add_manifest_reuses_existing_list() -> None:
    ...
    out = schema._properties["Manifest"]
...
def test_media_add_ingredient_reuses_existing_list() -> None:
    ...
    out = schema._properties["Ingredients"]
    assert isinstance(out, list)
    assert out == [first, second]
...
def test_exif_typed_struct_set_value_without_set_property_name() -> None:
    ...
    schema._typed_struct_set("OECF", sentinel)
    assert schema._properties["OECF"] is sentinel
