 ...
 * the thick-stroke geometry operators — line cap (``J`` 0 butt / 1 round /
 * 2 projecting-square), line join (``j`` 0 miter / 1 round / 2 bevel),
 * miter limit (``M``), and dash patterns (``d [array] phase``)
 * (PDF 32000-1 §8.4.3.3–§8.4.3.6).
 *
 * Usage: java -cp <pdfbox-app.jar>:<build> StrokeGeometryProbe input.pdf pageIndex
 ...
 * ``ImageMaskProbe.java`` so a probe swap won't shift values; this probe
 * exists as a named, dedicated entry point so the stroke-geometry parity
 * surface owns its own probe (per the wave brief). Comma-separated output
 * matches ImageMaskProbe so accidental whitespace edits can't silently
 * drift the parser.
 */
public final class StrokeGeometryProbe {
